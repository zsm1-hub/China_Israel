...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Case='HIT2d'; % wave
nparticles=324; % numbers of particles
days=89.5;  % days
seconds=0.05;  % days
...
max_distance = 6.28; % 最大距离
growth_factor = 1.5; % 分箱增长因子 (10%)

% 计算分箱边界
current = min_resolution;
dist_bin = [0, current]; % 从0开始，第一个分箱边界为最小分辨率

% 对数均匀增长
while current < max_distance
    current = current * growth_factor; % 每个分箱增长10%
    dist_bin(end+1) = current;
end

% 确保最后一个分箱不超过最大距离
dist_bin(end) = min(dist_bin(end), max_distance);
