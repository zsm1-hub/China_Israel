function [R,Y,Y_std,kf,dkf,Vt,eps,Fk,Fk_uncertainty,eps_error,...
    optimal_fac,optimal_po,...
    Th_Lag,Th_Lag_std]=Fk_fitting_SF3_Bayesian_RLS_Lcurve_uncertainty(Case,...
    nparticles,timerange,ini,mindist,maxdist,dot)
% input
...
% 不确定性分解
[Fk_total_std, Fk_epistemic_std, ...
    Fk_aleatoric_std] =decompose_Fk_uncertainty(Cxx, H, A, W, P);
% FxxP = sqrt(diag(errorsFlux(P, H)));
% FxxD = sqrt(diag(errorsFlux(Cxx_alternative-P, H)));
Fk_uncertainty.total = Fk_total_std;
Fk_uncertainty.epistemic = Fk_epistemic_std; 
Fk_uncertainty.aleatoric = Fk_aleatoric_std;
