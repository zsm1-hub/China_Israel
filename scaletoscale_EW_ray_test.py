...
#w->div, e->rot
@ray.remote
def tauijVij(vd, vdw, vde, gd, varlist, n):
    #w->div, e->rot
        od = {}
        r = n*gd['dx']
        ...
        for var in varlist:
            od[var]= np.squeeze(np.nanmean(locals()[var]))
        return od 

def tau_scales(vd, vdw, vde, gd, varlist):
    r = tN.Zeros(N0)
    #Tm = t:f
    od = {}
    for var in varlist:
    ...
    for I, valdict in enumerate(dictarr):
        for var in varlist:
            od[var][I] = valdict[var]
            #print(var, valdict[var])
        r[I] = Narray[I]*gd['dx'] 
    #w->div, e->rot
#        for var in varlist:
...
try:
        ...
        simul = sys.argv[4]
        itime_offset = int(sys.argv[5])
        NtEW = int(sys.argv[6])
except:
        print('Usage python scaletoscale_EW.py grd[2km/500] season depth simul[hf/smooth] initial_time_step last_time_step[-1 for all]')
        sys.exit(0) 
...
helm_flag = ncew.helmholtz_status
print(helm_flag)

if helm_flag  == 'yes':
    EddyOpt = ncew.EddyOpt
    WaveOpt = ncew.WaveOpt
    print(EddyOpt)
    print(WaveOpt)
...
outfile = 's2sEddyWave'
#if opt =='nohelm' else 's2sEddyWaveRotDiv'
...
Nt0 = 0
if NtEW == -1:
    NtEW = ncew.dimensions['time'].size
print(NtEW)
...
nco.filterEddyTime = filtE
nco.filterWaveTime = filtW
nco.helmholtz_status = helm_flag
...
# itime_offset=0
atime = 0
for itime in range(Nt0, NtEW,1):
    print(f'itime: {itime}')
    # modified by zsm
    # vd  = loadvd(nch, ncz, itime + itime_offset)
    vd  = loadvd(nch, nch, itime + itime_offset)
    #w->div, e->rot
    vde, vdw = loadvdEW(ncew, itime)
    filtscale, od = tau_scales(vd, vdw, vde, gd, varlist)
    for var in varlist: 
