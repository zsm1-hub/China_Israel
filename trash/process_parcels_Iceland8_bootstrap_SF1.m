...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Case='wave'; % wave
nparticles=1089; % numbers of particles
days=89.5;  % days
...
clear SF1 SF1_mean SF1_stderr

num_boot = 1000;
SF1 = zeros(length(dist_axis), num_boot);
SF1_mean = zeros(length(dist_axis),1);
SF1_stderr = zeros(length(dist_axis),1);
...
for i = 1:length(dist_axis)
    ...
    if npairs_sep(i)>10
        ...
        SF1_samp = blocks_dul + blocks_dut;
        ...
        SF1(i,:) = bootstrp(num_boot, @(x)mean(mean(x,2),1), SF1_samp);
    ...
    SF1_mean(i) = mean(SF1(i,:));
    SF1_stderr(i) = std(SF1(i,:)); % boot strap std err is the std of bs estimates
    
end
toc
outputname=[input_dir,Case,'_pars_P',num2str(nparticles),'T',num2str(timerange(end)),...
    ini,'bootstrap_SF1.mat']
% [input_dir,'wave_pars_P',num2str(nparticles),'T',num2str(days),'days.nc'];
save(outputname,'SF1','SF1_mean', 'SF1_stderr', 'dof',...
