...
for ii=1:length(nparticles)
    ...
    figure(5)
    subplot(2,2,1)
    b1=semilogx(filtscale./1e3,Thm_Eulerian,'LineWidth',1.5,'Color',[.7,.7,.7]);hold on
    ...
    [SpecFlux_E,Vt_E,ebs_E,kf_E,lf_E]=Fk_fitting_SF3(SF3(1:dot)',r(1:dot),1,200e3,'log','RLS',lambda);
    b2=semilogx(1./kf_E(dstr:end)./1e3,SpecFlux_E(dstr:end),'LineWidth',1.5, ...
    'Color','k');

     A{ii}=semilogx(1./kf./1e3,nanmean(SpecFlux,2),'Marker','x','Color',colors{ii}, ...
        'LineWidth',1.5);
    ...
    subplot(2,2,2)
    eval(['load ',Case,'_pars_P',num2str(nparticles(ii)),'T89.5daysCG_Lag_spectukey',ini,'.mat']);
    A{ii}=semilogx(filtscale./1e3,Th','LineWidth',1.5,'Color',colors{ii},'LineStyle','--');
    hold on
    if strcmpi(Case, 'wave')
        cgname='s2sflux_spec_hf.0002.nc';
        Case1='HF'
    ...
    ylabel('m^{2}/s^{3}')
    set(gca,'fontsize',16,'FontWeight','b')
    clear SpecFlux*;clear Vt;clear ebs;
    
    % subplot(2,2,3)
    % for tt=1:size(SF3,2)
    %      [SpecFlux(:,tt),Vt(:,tt),ebs(:,tt),kf,lf]=Fk_fitting_SF3(SF3(dstr:end,tt),dist_axis(dstr:end), ...
    %          1,300e3,'log','NNLS',lambda);
    % end
    % 
    % for i = 1:size(SpecFlux,1)    
    %     CI_SpecFlux(:,i) = prctile(SpecFlux(i,:), [95,5]);
    ...
    % 
    % subplot(2,2,4)
    % std1=std(ebs(1:end-1,:), 0, 2, 'omitnan');
    % x_fill = [1./kf./1e3, fliplr(1./kf./1e3)];
    % % y_fill = [CI_SpecFlux(1,:), fliplr(CI_SpecFlux(2,:))];
    % y_fill = [(nanmean(ebs(1:end-1,:),2)+std1)', fliplr((nanmean(ebs(1:end-1,:),2)-std1)')];
    % 
    % semilogx(1./kf./1e3,nanmean(ebs(1:end-1,:),2),'Marker','x','Color',colors{ii},'LineWidth',1.5)
    % hold on
    % fill(x_fill, y_fill, colors_rgb{ii}, 'FaceAlpha', 0.1, 'EdgeColor', 'none');
    % grid on
    % xlim([1e3,1e6]./1e3);
...
figure(5)
subplot(2,2,1)
legend(['b1','b2'],{'Eul CG','Eul SF3-fitting fk'})
title('Eulerian CG v.s. SF3-fitting fk')
subplot(2,2,2)
% legend(['a1','a2','a3','a4'],{'P289','P625','P2500','P15376'})
title('Eulerian CG v.s. Lag CG')
%%
...
figure(6)
rescale=2;
semilogx(1./kf./1e3.*rescale,fk15376_w,'Marker','x','Color',colors{ii}, ...
        'LineWidth',1.5);hold on
semilogx(1./kf./1e3.*rescale,fk15376_nw,'Marker','o','Color',colors{ii}, ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Case='wave'; % wave
nparticles=[289,625,2500,15376,900]; % numbers of particles
% nparticles=[289]; % numbers of particles
...
for ii=1:length(nparticles)
    ...
    figure(5)
    subplot(2,2,1)
    semilogx(1./kf./1e3,nanmean(SpecFlux,2),'Marker','x','Color',colors_rgb{ii}, ...
        'LineWidth',1.5);hold on
    if strcmpi(Case, 'wave')
        cgname='s2sflux_spec_hf.0002.nc';
        Case1='HF'
...
for ii=1:length(nparticles)
    ...
    % semilogx(1./kf./1e3,nanmean(SpecFlux,2),'Marker','x','Color',colors{ii}, ...
    %     'LineWidth',1.5);hold on
    % fname{ii,:}=['nowave_pars_P',num2str(nparticles(ii)),'T',num2str(days),'days.nc'];
    % eval(['load ',fname{ii}(1:end-3),'SF123_alltime',ini,'.mat']);
    % 
    % SF3=(SF3lll_time+SF3ltt_time);
    % dstr=14;
    ...
    %          1,300e3,'log','RLS',lambda);
    % end
    % semilogx(1./kf./1e3,nanmean(SpecFlux,2),'Marker','o','Color',colors{ii}, ...
    ...
    eval(['load ',Case,'_pars_P',num2str(nparticles(ii)),'T89.5daysCG_Lag_spectukey',ini,'.mat']);
    semilogx(filtscale./1e3,Th','LineWidth',1.5,'Color',colors{ii},'LineStyle','-');hold on
    eval(['load ','nowave_pars_P',num2str(nparticles(ii)),'T89.5daysCG_Lag_spectukey',ini,'.mat']);
...
cgnamew='s2sflux_spec_hf.0002.nc';

cgnamenw='s2sflux_spec_smooth.0002.nc';

% cgname='s2sflux_spec_smooth.0002.nc';
ncdisp(cgnamew)
Thm_Eulerianw=ncread(cgnamew,'Thm');
filtscale=ncread(cgnamew,'filtscale');
Thm_Euleriannw=ncread(cgnamenw,'Thm');

semilogx(filtscale./1e3,Thm_Eulerianw,'LineWidth',1.5,'Color',[.7,.7,.7],'LineStyle','-');hold on
semilogx(filtscale./1e3,Thm_Euleriannw,'LineWidth',1.5,'Color',[.7,.7,.7],'LineStyle','--');
grid on
ylim([-0.5e-7,0.5e-7]);
xlim([1e3,1e6]./1e3);
xlabel('km')
...
b2=semilogx(1./kf_E(dstr:end)./1e3,SpecFlux_E(dstr:end),'LineWidth',1.5, ...
'Color','k');hold on
eval(['load ','nowave_Eulerian_SF3.mat'])
N=287;dstr=6;dot=202;rescale=2;xscale=xscale.*sqrt(2);
[r,SF3,S3L1,S3T1]=calc_radial(S3L1,S3T1,N,xscale);
[SpecFlux_E,Vt_E,ebs_E,kf_E,lf_E]=Fk_fitting_SF3(SF3(1:dot)',r(1:dot),1,200e3,'log','RLS',lambda);
b2=semilogx(1./kf_E(dstr:end)./1e3,SpecFlux_E(dstr:end),'LineWidth',1.5, ...
