...
days=89.5;  % days
seconds=0.2;  % days
dt=1e-3; % s  Advection_RK4 delta_t drift时间间隔
...
num_bins = 50; % 总箱数
log_min = log10(1e-3); % 1mm
log_max = log10(6.28); % 6.28m
...
for jj=1:length(np)
    ...
    % 集合之前分开计算的chunk
    ii=1;
    eval(['load ',input_dir,Case,'_pars_P',num2str(npart),'T0.2secondschunk',num2str(ii,'%02d'),'SF123.mat']);
    ...
    if remainder>0
        eval(['load ',input_dir,Case,'_pars_P',num2str(npart),'T0.2secondschunk',num2str(ii+1,'%02d'),'SF123.mat'])
...
clear 
load HIT2d_pars_P2500T0.2secondsSF123.mat
SF3=(SF3lll_time+SF3ltt_time);

loglog(dist_axis,abs(SF3), 'linewidth',2);
hold on
loglog(dist_axis,-(SF3),'marker','+', 'linewidth',2);

loglog(dist_axis,(SF3),'marker','o', 'linewidth',2);
