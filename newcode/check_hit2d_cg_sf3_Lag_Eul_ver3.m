...
% kf1=fliplr((kf1(1:1:end))');
% kf2=[kf1(1:10:50),kf1(51:2:end)];
% kf1=kf2;
% kf1 = logspace( log10(1/3.32), log10(1/0.04), 10).*2.*pi ;
% 
% [SpecFlux_E,Vt_E,ebs_E,kf_E,lf_E]=Fk_fitting_SF3(SF3(1:dot)',r(1:dot),0.04,3.32, ...
%     'fuc','RLS',lambda,kf1);
% [SpecFlux_E2,Vt_E2,ebs_E2,kf_E2,lf_E2]=Fk_fitting_SF3(SF3(1:dot)',r(1:dot),0.04,3.32, ...
%     'fuc','RLS',lambda,kf1./2./pi);

kf1 = logspace( log10(1/6.32), log10(1/0.03), 12).*2.*pi ;

[SpecFlux_E,Vt_E,ebs_E,kf_E,lf_E]=Fk_fitting_SF3(SF3(1:dot)',r(1:dot),0.01,10, ...
    'fuc','RLS',lambda,kf1);
[SpecFlux_E2,Vt_E2,ebs_E2,kf_E2,lf_E2]=Fk_fitting_SF3(SF3(1:dot)',r(1:dot),0.01,10, ...
    'fuc','RLS',lambda,kf1./2./pi);
...
load HIT2d_pars_P62500T150timeavg7.mat
r_10000=dist_axis;
SF3_10000=SF3_mean;
Th_10000=nanmean(Th_all,2);
filtscale_L=filtscale(2:end);

% kf2= logspace(log10(1/4), log10(1/0.012), 11).*2.*pi;
% kf2= logspace(log10(1/4), log10(1/0.03), 9).*2.*pi;
kf2 = logspace( log10(1/6.32), log10(1/0.03), 12).*2.*pi ;

[SpecFlux_L10000,Vt_L10000,ebs_L10000,...
    kf_L,lf_L]=Fk_fitting_SF3_Lcurve(SF3_10000, ...
    r_10000,str1,en1, ...
    'fuc','RLS',lambda_opt_idx,kf2,0);
% [SpecFlux_L10000,Vt_L10000,ebs_L10000,...
%     kf_L,lf_L]=Fk_fitting_SF3_Lcurve(SF3_10000, ...
%     r_10000,str1,en1, ...
%     'fuc','RLS',lambda_opt_idx,kf_E,0);
...
save('HIT2d_fk_eps_fitting_plot_for_py_ver3.mat')